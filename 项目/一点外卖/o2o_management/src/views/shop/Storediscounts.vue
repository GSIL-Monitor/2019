<template>
  <div class="storediscounts container">
    ...
    <div class="addBottom">
      <div class="box1">
        ...
        <div class="boxContent">
          ...
          <p>
            ...
            <router-link
              class="setBtn"
              :to="({ name: 'Userdefinediscount',params: { busShopId: form.busShopId } })"
      ...
      <h5><el-button type="primary"  @click="submit" >{{ $t("BTN.Sub") }}</el-button></h5>
    </div>
    ...
    >
      ...
      <span
        slot="footer"
        class="dialog-footer"
      ><el-button type="primary" @click="discountsDialogVisible = false" >{{ $t("BTN.Back") }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  mounted () {
    ...
    //   优惠查询
    shop.shopDiscountsQuery(this.form.busShopId).then(res => {
      if (res) {
        this.form = res[0]
        if (res[0].fullMoney) {
          this.radio = '1'
        } else if (res[0].fullDiscount) {
          this.radio = '2'
        } else {
          this.radio = null
        }
  ...
  methods: {
    submit () {
      shop.updShopDiscounts(this.form).then(res => {
        this.$message({
          message: 'success',
          type: 'success'
        })
