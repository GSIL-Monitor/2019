...
<script>
import { upImg, shop } from '@/api/index.js'
export default {
  props: ['uploadUrl'],
  data () {
    return {
  ...
  mounted () {
    this.busShopCredential.busShopId = this.$route.query.busShopId
    this.init()
  },
  methods: {
    init () {
    // 店铺资质查询
      shop.busShopCredential(this.busShopCredential.busShopId).then(res => {
        console.log(res)
        if (res) {
          this.query = res
          this.isShow = true
    ...
    imgPreview (file, imgCode, callback) {
      ...
      upImg(this.param).then(res => {
        console.log(res)
        let fileUrl = JSON.parse(res).fileUrl
        if (imgCode === 1) {
    ...
    submit () {
      ...
      shop.addBusShopCredential(this.busShopCredential).then(res => {
        this.init()
      })
    }
  }
