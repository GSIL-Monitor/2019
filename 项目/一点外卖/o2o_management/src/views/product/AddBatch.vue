<template>
  <div class="addBatch container">
    ...
    <div class="addContent">
      <p>{{ $t("GOOD2.TXT1") }}<span>{{ $t("GOOD2.TXT2") }}</span><el-button type="primary"  @click="download">{{ $t("GOOD2.Download") }}</el-button></p>
      {{ $t("GOOD2.TXT3") }}
      ...
      <el-button class="btn addBatchBtn" @click="sendExcel" type="primary">{{ $t("BTN.Uplode") }}</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    sendExcel () {
      ...
      good.productExAdd(param).then(res => {
