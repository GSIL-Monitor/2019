      ...
      :header-cell-class-name='tableHeadRowClassName' :row-class-name="tableRowClassName" style="width: 600px">
        ...
        <el-table-column fixed="right" :label="$t('BTN.Handle')" width="120">
          <template slot-scope="scope">
            <router-link :to="{name:'AddCate',params:{'id':scope.row.id}}">{{ $t("BTN.Edit") }}</router-link> |
...
<script>
import { delBusProductCategory, querybusProductCategory } from '@/api/index.js'
export default {
  data () {
    return {
      loading: false,
  ...
  methods: {
    ...
    initData (num) {
      ...
      querybusProductCategory(this.serData).then(res => {
        if (!res.code) {
          this.tableData = res.data.query
          this.page.pageNum = res.data.count
        }
      })
      setTimeout(() => {
        this.loading = false
    ...
    // 删除
    del (id) {
      delBusProductCategory({id: id}).then(res => {
